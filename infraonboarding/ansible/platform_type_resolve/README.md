...
- generated inventory host data:
  - `endpoint`
- optional `reservation` when appliance scoping is needed
...
- claim candidate assembly prefers the prepared per-endpoint credential when present and
  otherwise falls back to the run-scoped desired credential list
