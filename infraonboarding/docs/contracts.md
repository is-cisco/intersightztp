...
- it prefers run-scoped desired credentials during claim-candidate assembly instead of
  carrying extra per-host credential wrappers through the dynamic inventory
...
- current verification confirms the grain reaches appliance-side `DeviceClaims`
  creation successfully; those claim records may still be in `started` state when
  the grain returns
